...
FR-3's multi-region resilience is proven by configuration and written explanation, not a live region kill (see SRS.md FR-3). Run `vector_index_spike.py --keep` first to leave seeded rows in place, then follow the write/read check in `RUNBOOK.md` §1 against those rows.
